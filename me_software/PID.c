...
void transfer(double act_speed, double volt, double time_step, double act_accel) {
	static double old_volt;
	new_speed = act_speed + time_step*act_accel + time_step*(0.86*((volt-old_volt) / time_step) + 0.53*volt);
	new_accel = -0.29*time_step*act_speed + (1-1.4*time_step)*act_accel + time_step*(0.86*((volt-old_volt) / time_step) + 0.53*volt);
	old_volt = volt;
}
...
int main(){
	...
	double output_volt;
	double act_accel = 0;
	fprintf(stderr, "Input desired speed:");
	scanf("%lf", &des_speed);
	fprintf(stderr, "Input actual speed:");
	scanf("%lf", &act_speed);
	...
	fprintf(stderr, "Settling time: ");
	fprintf(stderr, "%lf", time/1000);
