...
int stepper_simple()
{
	...
	__int64 curr_pos;
	const char *err;
	double maxAccel, maxVel, maxCurr;
	long long int maxPos, minPos;
	int stopped;
	...
	CPhidgetStepper_getAccelerationMax(stepper, 0, &maxAccel);
	CPhidgetStepper_setAcceleration(stepper, 0, 1350000/2);
	CPhidgetStepper_getVelocityMax(stepper, 0, &maxVel);
	CPhidgetStepper_setVelocityLimit(stepper, 0, 10000);
	CPhidgetStepper_getCurrentLimit(stepper, 0, &maxCurr);
	CPhidgetStepper_setCurrentLimit(stepper, 0, 1.63);
	CPhidgetStepper_getPositionMax(stepper, 0, &maxPos);
	CPhidgetStepper_getPositionMin(stepper, 0, &minPos);


	printf("Max Acceleration: %f\n", maxAccel);
	printf("Max Velocity: %f\n", maxVel);
	printf("Max Current: %f\n", maxCurr);
	printf("Max Position: %lld\n", maxPos);
	printf("Min Position: %lld\n", minPos);
	...
	//Step 2: Position 200
	printf("Move to position. Press any key to Continue\n");
	getchar();

	printf("Type number in 16 counts or whatever\n");
	while (1)
	{
		scanf("%d", &x);
		CPhidgetStepper_setTargetPosition (stepper, 0, x);
		printf("Type number in 16 counts or whatever\n");
		getchar();
	}

	printf("Move to position 0. Press any key to Continue\n");
	getchar();
